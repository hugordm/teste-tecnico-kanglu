...
- **Firecrawl** (padrão): busca as fontes via **API direta** (`POST /v2/search`, timeout curto de **12s**) e traz o conteúdo de cada página em **markdown**. Ruído (Instagram/YouTube/TikTok + dumps de documento tipo Scribd) sai na query (`-site:`), **sem** tocar no LinkedIn. As fontes passam por **descarte de índices**, **relevância por conteúdo** (off-topic como poliéster/portos é cortado), **idioma** (fonte não-portuguesa descartada — espanhol vazava termos) e **host/ano antigo** (ver `lib/relevance`). O sistema filtra concorrentes e o **modelo escolhido escreve** a partir do que sobrou.
...
- **Fallback automático**: se o Firecrawl falhar (erro, limite de crédito, timeout de 12s) **ou** não sobrar fonte, a busca cai no **Sonar** — que também passa por um pós-filtro (host/ano/índice/relevância por URL). No cron, uma **guarda de orçamento** não inicia o fallback se já passou de 35s, para não estourar os 60s da função. Só se o Sonar também falhar vira erro (`502`).
